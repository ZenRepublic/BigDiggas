		...
		for tile in tiles:
			if tile!=null:
				if tile_destroy_particles_scn!=null:
					var particles:GPUParticles2D = tile_destroy_particles_scn.instantiate()
...
func explode_animation() -> void:
	...
	game_manager.shockwave_manager.send_shockwave(self.global_position,player.cam)
	
	if explode_particles_scn!=null:
		var particles:GPUParticles2D = explode_particles_scn.instantiate()
		get_tree().root.get_node("MiningMinigame/ParticlesLayer").add_child(particles)
		#add_child(particles)
