...
func move_item_to_slot(item:Node2D,visual:Texture2D,inv_data:InventoryItem) -> void:
	...
	canvas_item.global_position -= Vector2(visual.get_width()/2.0, visual.get_height()/2.0)*player.cam.zoom
	canvas_item.scale *= player.cam.zoom

	audio_player.stream = inv_data.move_sound
	audio_player.play()
	
	var move_position:Vector2 = get_item_move_position(inv_data)
	...
	tween.parallel().tween_property(canvas_item, "scale", canvas_item.scale*3.0, 0.15).set_trans(Tween.TRANS_LINEAR).set_ease(Tween.EASE_IN_OUT)
